namespace DotNet.Testcontainers.Containers
{
  ...
  public sealed class CouchbaseTestcontainer : TestcontainerDatabase
  {
    ...
    public async Task<long> CreateBucket(string bucket, int memory = 128)
    {
      ...
      var result = await this.ExecAsync(new[] { "/bin/sh", "-c", createBucketCommand });
    ...
    public async Task<long> FlushBucket(string bucket)
    {
      ...
      var result = await this.ExecAsync(new[] { "/bin/sh", "-c", flushBucketCommand });
      return result.ExitCode;
    }
  }
}
